...
const FileAnalyzer = () => {
  ...
  const analyzeDocument = async (text: string) => {
    try {
      ...
      const aiService = new AIServiceFactory(
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are an AI assistant that analyzes documents and extracts key information. Provide a thorough analysis with a summary, key topics, sentiment, and any important details.",
              },
              {
                role: "user",
      ...
      const analysis =
        data.choices[0]?.message?.content || "Analysis could not be generated";
        ...
        <CardContent className="p-5">
          ...
          {!credentials.openaiApiKey && (
            <Alert variant="warning" className="mt-4">
              ...
              <AlertDescription>
                Please set your OpenAI API key in the settings to enable
                document analysis.
              </AlertDescription>
            </Alert>
          )}
